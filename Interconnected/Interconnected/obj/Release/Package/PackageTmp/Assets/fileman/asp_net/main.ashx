...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
	...
	protected void ListDirTree(string type)
	{
        ...
        for (int i = 0; i < dirs.Count; i++)
        {
            ...
            string floder = "{\"p\":\"/" + dir.Replace(localPath, "").Replace("\\", "/") + "\",\"f\":\"" + GetFiles(dir, type).Count.ToString() + "\",\"d\":\"" + Directory.GetDirectories(dir).Length.ToString() + "\"}";
            ListDirector.Add(floder);
            
        }
        for (int j = 0; j < ListDirector.Count; j++)
        {
            _r.Write(ListDirector[j]);
            if (j < ListDirector.Count - 1)
                _r.Write(",");
        }
